    ...
    for (let file in samples) {
        ...
        describe(filename, function () {
            ...
            test('renders with the MaterialUI decorator', async function () {
                ...
                util.traverseForm(form, function (form) {
                    let key = form;
                    ...
                    const formSchema = util.findSchema(key, schema);
                    if (
                        /^date/.test(form.type) ||
                        (formSchema &&
                            formSchema.type === 'string' &&
                            /^date/.test(formSchema.format))
                    ) {
                        const set = util.valueSetter(model, schema);
                        model = set(key, '1970-01-01T00:00:00Z');
                    }
                });
