...
describe('defaultForSchema', function() {
    ...
    describe('when given one type', function() {
        test('returns an empty value of the type', function() {
            ...
            expect(util.defaultForSchema({ type: 'array', items: { type: 'string' } })).toMatchObject([]);
            expect(util.defaultForSchema({ type: 'array', items: [{ type: 'string' }] })).toMatchObject(['']);
            expect(util.defaultForSchema({ type: 'object' })).toMatchObject({});
            expect(util.defaultForSchema({ type: 'object', properties: { test: { type: 'string' } } })).toMatchObject({ test: '' });
    ...
    describe('when given many types', function() {
        test('returns an empty value of the preferredType', function() {
            ...
            expect(util.defaultForSchema({ type: ['number', 'string'] })).toBe(0.0);
            expect(util.defaultForSchema({ type: ['integer', 'string'] })).toBe(0);
            expect(util.defaultForSchema({ type: ['boolean', 'number'] })).toBe(false);
            expect(util.defaultForSchema({
                type: ['array', 'object'],
                items: { type: 'string' },
                properties: {
                    test: { type: 'string' }
            ...
            expect(util.defaultForSchema({ type: ['object', 'array'] })).toMatchObject({});
            expect(util.defaultForSchema({ type: ['unknown', 'integer'] })).toBeUndefined();
        });
    })
});
describe('assertType', function() {
    test('nullifies falsey values if allowed', function() {
        ...
        expect(util.assertType({ type: ['boolean', 'null'] }, false)).toBeNull();
    });
    ...
    describe('when the preferred type is an integer', function() {
        describe('when the type is allowed', function() {
            test('allows the value', function() {
                expect(util.assertType({ type: ['integer', 'string'] }, 'test')).toBe('test')
                expect(util.assertType({ type: ['integer', 'boolean'] }, false)).toBe(false);
            })
        })
    ...
    describe('when the preferred type is a number', function() {
        describe('when the type is allowed', function() {
            test('allows the value', function() {
                expect(util.assertType({ type: ['number', 'string'] }, 'test')).toBe('test')
                expect(util.assertType({ type: ['number', 'boolean'] }, false)).toBe(false);
            })
        })
        describe('when the type is a string', function() {
    ...
    describe('when the type is allowed', function() {
        test('returns the value unchanged', function() {
            expect(util.assertType({ type: ['string', 'boolean'] }, true)).toBe(true);
        });
    });
    describe('when the type is not allowed', function() {
        test('returns defaultForSchema', function() {
...
describe('findSchema', function() {
    test('iterates over a schema following keys', function() {
        ...
        expect(util.findSchema(['test'], schema)).toMatchObject({ type: 'string' });
        expect(util.findSchema(['tuple', 1], schema)).toMatchObject({ type: 'number' })
    })
})
describe('valueGetter', function() {
    test('traverses objects to retrieve a value', function() {
        const get = util.valueGetter({ test: { property: 'a' } }, {
            type: 'object',
            properties: {
                test: {
                    type: 'object',
...
describe('valueSetter', function() {
    test('traverses objects to set a value', function() {
        const set = util.valueSetter({ test: { property: 'a' } }, {
            type: 'object',
            properties: {
                test: {
                    type: 'object',
        ...
        expect(set(['test', 'property'], 'b')).toMatchObject({ test: { property: 'b' } });
    })
    ...
    test('uses defaultForSchema when traversing undefined trees', function() {
        ...
        expect(set(['test', 'property'], 'a')).toMatchObject({ test: { property: 'a' } });
    });
